...
import { ArrowUp, Paperclip, Square, X, StopCircle, Mic, Scale, FileSearch, Apple } from "lucide-react";
      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              "rounded-3xl border bg-background/80 backdrop-blur-sm p-2 shadow-lg transition-all duration-300",
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  const { onSend = () => {}, isLoading = false, placeholder = "Ask about food nutrition, compare products, or plan your diet...", className } = props;
  ...
  const handleToggleChange = (value: string) => {
    if (value === "comparison") {
      // Direct redirect - immediately trigger compare mode
      onSend("[Compare:]", []);
      setShowComparison(false);
      setShowInfo(false);
      setShowDiet(false);
    } else if (value === "info") {
      // Direct redirect - immediately trigger info mode
      onSend("[Info:]", []);
      setShowInfo(false);
      setShowComparison(false);
      setShowDiet(false);
    } else if (value === "diet") {
      setShowDiet((prev) => !prev);
      setShowComparison(false);
      setShowInfo(false);
    }
  };
  ...
  const handleSubmit = () => {
    if (input.trim() || files.length > 0) {
      let messagePrefix = "";
      if (showDiet) messagePrefix = "[Diet Plan:";
          ...
          <PromptInputTextarea
            placeholder={
              showComparison
                ...
                : showInfo
                ? "Upload a product image to analyze ingredients..."
                : showDiet
                ? "Tell me about your goals and I'll create a diet plan..."
                : placeholder
            }
            className="text-base"
          />
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          ...
          >
            ...
            <div className="flex items-center">
              <button
                type="button"
                onClick={() => handleToggleChange("comparison")}
                className={cn(
              ...
              >
                <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                  <motion.div
                    animate={{ rotate: showComparison ? 360 : 0, scale: showComparison ? 1.1 : 1 }}
                    whileHover={{ rotate: showComparison ? 360 : 15, scale: 1.1, transition: { type: "spring", stiffness: 300, damping: 10 } }}
                    transition={{ type: "spring", stiffness: 260, damping: 25 }}
                  >
                    <Scale className={cn("w-4 h-4", showComparison ? "text-cyan-500" : "text-inherit")} />
                  </motion.div>
                </div>
                <AnimatePresence>
                  {showComparison && (
                    <motion.span
                      ...
                      className="text-xs overflow-hidden whitespace-nowrap text-cyan-500 flex-shrink-0"
                    >
                      Compare
                    </motion.span>
                  )}
              ...
              <button
                type="button"
                onClick={() => handleToggleChange("info")}
                className={cn(
                  "rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8",
                  showInfo
              ...
              >
                <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                  <motion.div
                    animate={{ rotate: showInfo ? 360 : 0, scale: showInfo ? 1.1 : 1 }}
                    whileHover={{ rotate: showInfo ? 360 : 15, scale: 1.1, transition: { type: "spring", stiffness: 300, damping: 10 } }}
                    transition={{ type: "spring", stiffness: 260, damping: 25 }}
                  >
                    <FileSearch className={cn("w-4 h-4", showInfo ? "text-purple-500" : "text-inherit")} />
                  </motion.div>
                </div>
                <AnimatePresence>
                  {showInfo && (
              ...
              <button
                type="button"
                onClick={() => handleToggleChange("diet")}
                className={cn(
                  "rounded-full transition-all flex items-center gap-1 px-2 py-1 border h-8",
                  showDiet
              ...
              >
                <div className="w-5 h-5 flex items-center justify-center flex-shrink-0">
                  <motion.div
                    animate={{ rotate: showDiet ? 360 : 0, scale: showDiet ? 1.1 : 1 }}
                    whileHover={{ rotate: showDiet ? 360 : 15, scale: 1.1, transition: { type: "spring", stiffness: 300, damping: 10 } }}
                    transition={{ type: "spring", stiffness: 260, damping: 25 }}
                  >
                    <Apple className={cn("w-4 h-4", showDiet ? "text-emerald-500" : "text-inherit")} />
                  </motion.div>
                </div>
                <AnimatePresence>
                  {showDiet && (
